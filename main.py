...
async def handle_text(chat_id, user_id, texto):
    ...
    if len(partes) < 2:
        await send_message(chat_id, "⚠️ Escribe: `fecha productor`\nEj: `21/11/2025 MARIO PAMPAS`")
        return
    ...
    if not resultados:
        await send_message(chat_id, f"❌ Sin resultados para:\n📅 {fecha_str}\n👤 {productor_str}")
        return
...
async def handle_callback(chat_id, user_id, callback_data, callback_query_id):
    ...
    # Seleccionar análisis
        ...
        if callback_data == "analisis_todos":
            texto = "📄 *Análisis encontrados:*\n\n"
            for i, r in enumerate(resultados):
                texto += f"*{i+1}. {r['tipo_analisis']}*\n"
                texto += f"👤 {r['productor']}\n"
                texto += f"📅 {r['fecha']}\n"
                if r["reporte"]:
